...
const uint8_t* RandKeyGen(uint8_t key[16])
{
    ...
    for(int i = 0; i<16; i++)
    {
        key[i] = (uint8_t)(rand() % (maxi - mini + 1) + mini);
    }
...
void raddRoundKey(uint8_t *state, uint8_t *roundKey) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}

void raesRound(uint8_t *state, uint8_t *roundKey) {
    raddRoundKey(state, roundKey);
